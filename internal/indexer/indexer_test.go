...
func TestPollOnceIndexesSmallPage(t *testing.T) {
	...
	for n := int64(1); n <= 3; n++ {
		...
		hash := blockchain.CalculateBlockHash(previousHash, n, tx.ID)
		page.Blocks = append(page.Blocks, blockchain.Block{Height: n, Hash: hash, PreviousHash: previousHash, Transactions: []transaction.Signed{tx}})
		previousHash = hash
	}
...
func TestPollOnceIndexesEmptyBlock(t *testing.T) {
	...
	page := blockchain.BlockPage{
		Blocks:     []blockchain.Block{{Height: 1, Hash: blockchain.CalculateBlockHash("", 1), Transactions: []transaction.Signed{}}},
		NextHeight: 1,
	}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
